...
predict_flag = [];
answer = [];
filename = dir('task2_data');
filename = filename(3:end);
filenum = size(filename);
Tlc = 0.6;
image = imread('data/peppers.bmp');
[width, height] = size(image);

for i = 1:40
	pattern = load(['task2_data/' filename(i).name]);
	pattern = pattern.pattern;
	for j = -1:1
        res_image = image;
		if j ~= 0
			res_image = E_blind(image, pattern, j, 1);
		end
		k = i*3+j-1;
		answer(k) = j;
		predict(k) = D_LC(res_image, pattern);
		if predict(k) > Tlc
			predict_flag(k) = 1;
